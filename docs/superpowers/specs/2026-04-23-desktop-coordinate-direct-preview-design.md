...
- 前端在点击搜索时先做轻量原生格式校验：
  - 必须是 5 段
  - 第 2 段是十进制 seed
  - 第 5 段需为 `1..5` 位大写 base36，且解码后不能超出 11 槽 mixing 的有效范围
  - 校验失败时直接弹窗提示，不发送请求
- 前端通过后再发送原始坐标码，不在前端做最终语义解析
...
5. 第 3/4 段原样保留，但不进入当前仓库世界生成逻辑
6. 使用原始坐标直接进入 `AppRuntime::Generate()`，由 `CoordinateChanged()` 建立最终生成状态
7. 若原始坐标无法完成生成，则拒绝请求
